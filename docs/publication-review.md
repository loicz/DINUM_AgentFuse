...
**Mise à jour du 12 septembre 2026 — arrêt des services :** `python3 stop.py` fournit désormais l'arrêt ordonné des cinq services, avec contrôle des processus propres au dépôt, attente de sortie et conservation des données. La [procédure](../README.md#arrêter-la-démonstration) couvre aussi l'état des services, les délais et les arrêts répétés. Onze tests déterministes avec processus isolés ont réussi, notamment pour les anciens PID, un autre dépôt, un autre cluster et un backend qui refuse de sortir. Un cluster PostgreSQL neuf dans `/tmp` a été arrêté proprement deux fois par cette commande, avec conservation d'une donnée après redémarrage. Le sdist a été construit dans `/tmp`, puis sa commande d'arrêt exécutée après extraction ; aucun répertoire d'exécution n'est livré. Aucun nouvel essai du vrai modèle ni déploiement de production n'a été effectué pour cette modification. Le constat d'arrêt absent dans le tableau décrit la revue du 11 septembre ; les procédures de sauvegarde/restauration et de résolution des conflits de ports restent à compléter.
